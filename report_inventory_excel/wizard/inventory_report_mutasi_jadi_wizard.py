...
class InventoryExcelMutasiJadiExportSummary(models.TransientModel):
    _name = "inventory.excel.mutasi.jadi.export.summary"

    file = fields.Binary(
        "Click On Save As Button To Download File", readonly=True)
    name = fields.Char(
        "Name", size=32, default='Laporan Mutasi Barang Jadi.xls')


class InventoryExportMutasiJadiReportWizard(models.TransientModel):
    _name = 'inventory.export.mutasi.jadi.report.wizard'
    ...
    def print_inventory_export_report(self):
        ...
        query = """
                ...
                where """ + where_categ + """ and """ + where_product + """ and pc.report_type = 'barang_jadi'
        ...
        worksheet.write_merge(2, 2, 0, 2, "Mutasi Barang Jadi", xls_format.font_style(
            position='left', bold=1, fontos='black', font_height=300))
        ...
        res_id = self.env['inventory.excel.mutasi.jadi.export.summary'].create(
            {'fname': 'Laporan Mutasi Barang Jadi.xls', 'file': res})

        return {
            'type': 'ir.actions.act_url',
            'url': '/web/binary/download_document?model=inventory.excel.mutasi.jadi.export.summary&field=file&id=%s&filename=Laporan Mutasi Barang Jadi.xls' % (res_id.id),
            'target': 'new',
        }
