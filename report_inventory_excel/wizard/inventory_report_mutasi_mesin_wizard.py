...
class InventoryExcelMutasiMesinExportSummary(models.TransientModel):
    _name = "inventory.excel.mutasi.mesin.export.summary"

    file = fields.Binary(
        "Click On Save As Button To Download File", readonly=True)
    name = fields.Char(
        "Name", size=32, default='Laporan Mutasi Mesin dan Peralatan Perkantoran.xls')


class InventoryExportMutasiMesinReportWizard(models.TransientModel):
    _name = 'inventory.export.mutasi.mesin.report.wizard'
    ...
    def print_inventory_export_report(self):
        ...
        worksheet.write_merge(2, 2, 0, 2, "Mutasi Mesin dan Peralatan Perkantoran", xls_format.font_style(
            position='left', bold=1, fontos='black', font_height=300))
        ...
        res_id = self.env['inventory.excel.mutasi.mesin.export.summary'].create(
            {'fname': 'Laporan Mutasi Mesin dan Peralatan Perkantoran.xls', 'file': res})

        return {
            'type': 'ir.actions.act_url',
            'url': '/web/binary/download_document?model=inventory.excel.mutasi.mesin.export.summary&field=file&id=%s&filename=Laporan Mutasi Mesin dan Peralatan Perkantoran.xls' % (res_id.id),
            'target': 'new',
        }
