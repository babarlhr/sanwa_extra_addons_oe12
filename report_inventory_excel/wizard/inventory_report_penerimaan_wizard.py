...
class InventoryExcelPenerimaanExportSummary(models.TransientModel):
    _name = "inventory.excel.penerimaan.export.summary"

    file = fields.Binary(
        "Click On Save As Button To Download File", readonly=True)
    name = fields.Char("Name", size=32, default='laporan_pemasukan.xls')


class InventoryExportPenerimaanReportWizard(models.TransientModel):
    _name = 'inventory.export.penerimaan.report.wizard'
    ...
    def print_inventory_export_report(self):
        ...
        query = """
                ...
                AND ((sm.location_id != '12' AND sm.location_dest_id = '12')
                    OR (sm.location_id = '24' AND sm.location_dest_id = '26'))  
        ...
        worksheet.write(8, 3, "Tanggal", xls_format.font_style(
            position='center', bold=1, border=1, fontos='black', font_height=200, color='grey'))
        worksheet.write_merge(7, 7, 4, 5, "Bukti Penerimaan Barang", xls_format.font_style(
            position='center', bold=1, border=1, fontos='black', font_height=200, color='grey'))
        worksheet.write(8, 4, "Nomor", xls_format.font_style(
            position='center', bold=1, border=1, fontos='black', font_height=200, color='grey'))
        worksheet.write(8, 5, "Tanggal", xls_format.font_style(
            position='center', bold=1, border=1, fontos='black', font_height=200, color='grey'))
        worksheet.write_merge(7, 8, 6, 6, "Pemasok/Pengirim", xls_format.font_style(
            position='center', bold=1, border=1, fontos='black', font_height=200, color='grey'))
        ...
        res_id = self.env['inventory.excel.penerimaan.export.summary'].create(
            {'fname': 'Laporan Pemasukan Barang.xls', 'file': res})

        return {
            'type': 'ir.actions.act_url',
            'url': '/web/binary/download_document?model=inventory.excel.penerimaan.export.summary&field=file&id=%s&filename=Laporan Pemasukan Barang.xls' % (res_id.id),
            'target': 'new',
        }
        ...
        # 'view_type': 'form',
        # "view_mode": 'form',
        # 'res_model': 'inventory.excel.penerimaan.export.summary',
