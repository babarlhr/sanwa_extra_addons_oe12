...
class InventoryExcelMutasiExportSummary(models.TransientModel):
    _name = "inventory.excel.mutasi.export.summary"

    file = fields.Binary(
        "Click On Save As Button To Download File", readonly=True)
    name = fields.Char(
        "Name", size=32, default='Laporan Mutasi Bahan Baku.xls')


class InventoryExportMutasiReportWizard(models.TransientModel):
    _name = 'inventory.export.mutasi.report.wizard'
    ...
    def print_inventory_export_report(self):
        ...
        query = """
                ...
                where """ + where_categ + """ and """ + where_product + """ 
                    and pc.report_type = 'bahan_baku'
                    and pp.active = 't'
        ...
        worksheet.write_merge(2, 2, 0, 2, "Mutasi Bahan Baku dan Bahan Penolong", xls_format.font_style(
            position='left', bold=1, fontos='black', font_height=300))
        ...
        for val in hasil:
            ...
            saldo_akhir = round(saldo_akhir, 5)
        ...
        res_id = self.env['inventory.excel.mutasi.export.summary'].create(
            {'fname': 'Laporan Mutasi Bahan Baku.xls', 'file': res})

        return {
            'type': 'ir.actions.act_url',
            'url': '/web/binary/download_document?model=inventory.excel.mutasi.export.summary&field=file&id=%s&filename=Laporan Mutasi Bahan Baku.xls' % (res_id.id),
            'target': 'new',
        }
